...
const SAMPLE_MENU = [
  {
    ...
    price: 15000,
    categoryId: 5,
    imageUrl: "https://cdn-icons-png.flaticon.com/512/16999/16999650.png",
    isAvailable: true,
  },
  {
    id: 201,
    ...
    price: 65000,
    categoryId: 1,
    imageUrl: "https://cdn-icons-png.flaticon.com/512/2276/2276931.png",
    isAvailable: true,
  },
  {
    id: 301,
    ...
    price: 35000,
    categoryId: 3,
    imageUrl: "https://cdn-icons-png.flaticon.com/512/3055/3055861.png",
    isAvailable: true,
  },
];
...
export default function GuestMenu() {
  ...
  useEffect(() => {
    ...
    const syncOrderStatus = async () => {
      try {
        const serverOrders = await fetchAPI(
          SERVICES.ORDER,
          ...
          setMyOrders((prevLocalOrders) => {
            return prevLocalOrders.map((localOrder) => {
              const match = serverOrders.find(
                (so) => String(so.id) === String(localOrder.id)
              );
  ...
  useEffect(() => {
    if (isDataLoaded)
      localStorage.setItem("s2o_guest_cart", JSON.stringify(cart));
  }, [cart, isDataLoaded]);

  useEffect(() => {
    if (isDataLoaded)
      localStorage.setItem("s2o_guest_orders", JSON.stringify(myOrders));
  ...
  useEffect(() => {
    const loadMenu = async () => {
      ...
      if (rawData) {
        ...
        const uniqueCats = [
          "Tất cả",
          ...new Set(processedItems.map((i) => i.category)),
        ];
  ...
  const updateItemNote = (itemId, text) => {
    setCart((prev) =>
      prev.map((item) => (item.id === itemId ? { ...item, note: text } : item))
    );
  };
      ...
      <>
        ...
        >
          {displayCategories.map((cat) => {
            const itemsInCat = searchedItems.filter(
              ...
              {order.Items &&
                order.Items.map((item, idx) => (
                  ...
                  >
                    <span>
                      {item.quantity}x{" "}
                      {item.name || item.menuItemName || item.MenuItemName}
                    </span>
                    {item.note && (
                      <span
                        style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ margin: "5px 0 10px", opacity: 0.9, fontSize: 14 }}>
          📍 {tableName}
        </p>

        <div
          style={{ display: "flex", gap: 20, fontSize: 16, fontWeight: "bold" }}
        >
          <div
            onClick={() => setActiveTab("menu")}
            style={{
              paddingBottom: 5,
              cursor: "pointer",
              borderBottom: activeTab === "menu" ? "3px solid white" : "none",
          ...
          <div
            ...
            style={{
              ...
              cursor: "pointer",
              borderBottom:
                activeTab === "history" ? "3px solid white" : "none",
              opacity: activeTab === "history" ? 1 : 0.6,
            }}
          >
            Đã gọi ({myOrders.length})
          </div>
        </div>
      </div>
